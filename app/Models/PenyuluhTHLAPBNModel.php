...
class PenyuluhTHLAPBNModel extends Model
{
    ...
    {
        $db = Database::connect();
        $query = $db->query("select count(a.id) as jum, nama_dati2 as nama_kab from tbldasar_thl a left join tbldati2 b on b.id_dati2=a.satminkal where satminkal='$kode_kab'");
        $row   = $query->getRow();

        $query   = $db->query("select a.no_peserta, a.nama, a.tgl_update, d.nm_desa, j.deskripsi as kecamatan_tugas,
                                case a.penyuluh_di 
                                when 'kabupaten' then 
                                    case a.unit_kerja 
