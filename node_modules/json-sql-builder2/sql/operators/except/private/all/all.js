...
module.exports = {
	definition: all,
 	description: 'Specifies the `EXCEPT ALL` Operator.',
	supportedBy: {
				...
				Object: {
					'Basic Usage': function(sql) {
						return {
							...
							expectedResults: {
								sql: '((SELECT first_name FROM people) EXCEPT ALL (SELECT last_name FROM people))',
								values: {}
							}
						}
					},
					'Nested unions': function(sql) {
						return {
							...
							expectedResults: {
								sql: '(SELECT first_name FROM people) UNION ((SELECT last_name FROM people WHERE last_name LIKE $1) EXCEPT ALL (SELECT last_name FROM people WHERE last_name LIKE $2))',
								values: {
									$1: 'K%',
									$2: 'A%'
								}
