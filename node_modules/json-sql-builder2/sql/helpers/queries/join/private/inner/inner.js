...
class inner extends joinHelper {
	constructor(sql){
		super(sql, 'inner');
	}
}
		...
		Object: {
			'Basic Usage': function(sql) {
				return {
					...
					expectedResults: {
						sql: 'SELECT people.first_name, people.last_name, skills.description, skills.rate FROM people INNER JOIN (SELECT * FROM people_skills WHERE is_skill = $1) AS skills ON skills.people_id = people.people_id WHERE skills.rate > $2',
						values:{
							$1: 1,
							$2: 50
						}
			...
			'Oracle Basic Usage': function(sql) {
				return {
					...
					expectedResults: {
						sql: 'SELECT people.first_name, people.last_name, skills.description, skills.rate FROM people INNER JOIN (SELECT * FROM people_skills WHERE is_skill = $1) skills ON skills.people_id = people.people_id WHERE skills.rate > $2',
						values:{
							$1: 1,
							$2: 50
						}
